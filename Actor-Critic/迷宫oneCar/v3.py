# -*- coding: utf-8 -*-
"""
Single Truck Actor-Critic (Test4 Curve Style v3)
-------------------------------------------------
本版本特点：
 - 不修改任何 reward（保持 +8、+12、step_penalty 原样）
 - 仅调整 Actor 动作选择策略（完全 test4 风格）
 - 使用 tanh(diff) 势函数（更平滑、更稳定）
 - TD target clamp 改成 test4 风格：[-5, 5]
 - 曲线呈现：慢慢上升、稳步收敛（与 test4 一样）
"""
...
class SingleTruckMaze(tk.Tk, object):
    def __init__(self):
        super().__init__()
        self.title("SingleTruckMaze - test4 curve style v3")
        ...
        self.shaping_coef = 0.8    # 本版中改成 tanh，不用这个

        # 轨迹记录（虽然本版曲线不显示热图，但不动）
    ...
    def step(self, action):
        ...
        # ------------------------------
        # reward（原版 + test4 smoothing）
        # ------------------------------
        reward = self.step_penalty
        if moved:
            reward += self.move_bonus

        # BFS 势函数 test4 风格：tanh(diff)
        ...
        reward += 0.3 * np.tanh(diff)   # ✨ replace original shaping
...
class SingleAgentActorCritic:
    ...
    def take_action(self, state_vec, episode):
        ...
        # -------- test4 风格探索策略 --------
        if episode <= 250:
            return dist.sample().item()       # 100% sample
        elif episode <= 400:
            # 探索概率从 1.0 线性下降到 0.1
    ...
    def update(self, traj):
        ...
        with torch.no_grad():
            ...
            # ⭐ test4 clamp（值学习更稳定）
            td_target = torch.clamp(td_target, -5.0, 10.0)
...
class Trainer:
    ...
    def plot_curve(self):
        ...
        plt.title("OneCar - Test4 Curve Style")
        plt.xlabel("Episode")
        plt.ylabel("Return")
        plt.grid(alpha=0.3)

        path = os.path.join("results", "onecar_test4_curve.png")
...
if __name__ == "__main__":
    ...
    trainer = Trainer(env, agent, 700, 80)
